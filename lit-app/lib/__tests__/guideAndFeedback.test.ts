...
describe("Afternoon Check-In unlock time (gate #1 persistence input)", () => {
  ...
  it("computes 7 hours after the planned wake time", () => {
    expect(computeAfternoonUnlockLabel("8:00 AM", undefined)).toBe("3:00 PM");
  });

  it("prefers the planned wake time over the learned estimate", () => {
    expect(computeAfternoonUnlockLabel("7:00 AM", "9:00 AM")).toBe("2:00 PM");
  });
});
